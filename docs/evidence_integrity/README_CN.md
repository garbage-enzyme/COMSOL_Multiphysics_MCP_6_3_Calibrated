...
1. 把正式 artifact 保存在 configured owned artifact root 中。默认位置是
   `COMSOL_MCP_RUNTIME_DIR` 下的 `owned_artifacts`；若设置
   `COMSOL_MCP_ARTIFACT_WRITE_ROOT`，应使用一个绝对、仅 ASCII 的目录。
2. 不提供 evidence settings 文件时调用 `evidence_integrity_status`。四项检查均应
   显示 `enabled: true`、`source: default`，并且
...
| 输入错误后使用猜测 fallback | Bounded machine-readable failure | unknown field、错误类型、重复键、损坏 JSON、missing artifact、stale revision 和 unsupported schema 全部 fail closed | 有界 reason/error，不会静默变成 default-off |
...
`COMSOL_MCP_EVIDENCE_SETTINGS_PATH` 选择 settings 文件。MCP 不返回这个私有路径，
只报告环境变量名、每项 effective state、来源（`default` 或
`explicit_settings`）和 canonical settings fingerprint。

settings 文件不是必需的。环境变量不存在时，四项检查全部默认开启。如果需要文件，
从 [`default_settings.json`](default_settings.json) 开始：
...
只有显式 JSON boolean `false` 才能关闭某项检查。缺失的 key 仍然默认开启。例如
[`exploration_settings.json`](exploration_settings.json) 只关闭 exact summary-claim
verification：
...
loader 会在每次 status 或 guarded tool call 时读取当前文件，因此只修改 settings
不需要重启 MCP host。但恢复检查后，必须针对未改变的 artifacts 做一次**新的验证**；
不能升级较早的 unverified receipt。

unknown field、重复 JSON key、malformed JSON、wrong type、不可读文件或 unsupported
schema 都会阻止 formal verification，不会静默关闭检查。
...
| 现象 | 含义 | 安全的下一步 |
| --- | --- | --- |
| `configuration_state: invalid` | settings 损坏、含歧义、不可读或 unsupported | 修正文件；不要再添加一个配置来源来绕过错误 |
