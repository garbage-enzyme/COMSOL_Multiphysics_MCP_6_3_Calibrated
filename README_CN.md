...
| `desktop_shared` | 显式选择的 shared Desktop/attached-Server 工作流；要求 `COMSOL_MCP_ENABLE_SHARED_SERVER=true`、用户手动启动本地 Server、每次 attach 的用户确认、精确进程/listener 身份和精确模型采用；不会启动或终止外部 Server。 |
...
共享 Desktop/attached-Server 工作被隔离在默认关闭的 `desktop_shared` profile 和静态
`COMSOL_MCP_ENABLE_SHARED_SERVER=true` flag 后面。用户必须手动启动 COMSOL Server，
